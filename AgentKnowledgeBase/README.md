...
| 4 | `P0-DOC-02_instrument_datasheet_extract.md` | Every fact the manufacturer PDFs establish, with page citations — plus what they don't. |
| 5 | `command_register.md` | The only place a hardware command may come from. 0 approved entries today. |
| 6 | `instrument_profiles.yaml` | Machine-readable configuration the application loads. |
...
1. **Never invent a hardware command.** A command may be used only if it appears in `command_register.md` with a manual page citation for the exact model *and* firmware. Three of the four instruments are `BLOCKED` because their manuals are not here. Similarity to another model is not evidence.
